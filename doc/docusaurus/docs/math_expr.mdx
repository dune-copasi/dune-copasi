...
Every expression in `{math-expr}` is compiled independently. This means that different parser types may be used in the same model definition. This is helpful when the syntax of one parser is more convenient to write one expression. For example, the
...
```
In this example the reaction expression for `u` is interpreted with the `SymEngine` parser, however, the function `pulse` defined in the
parser context section is interpreted with the `ExprTk` parser. This mechanisim allows to compose parser capabilities as needed.
:::
::::
