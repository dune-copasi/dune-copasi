...
$\partial\Omega^k$ and space dimension $d=\{1,2,3\}$. Moreover, we require
...
We will distinguish individual scalar components within the same compartment with
a subscript, usually $i$, and components on different compartments with a
superscript, usually $k$. For example, $u_i^k$ is the $i$-th scalar component on
the $k$-th compartment. Furthermore, we will denote bold letters to refer to
all $N^k$ components in the $k$-th compartment, i.e, $\bm{u}^k:=[u^k_1,\dots,u_{N^k}^k]^T$.

### Transport

We then characterize transport with diffusive and advective flux operators.

#### Diffusive flux operator
We then characterize the diffusive transport,
...
A reaction term is the transformation of one component to another, i.e,
...
$u^k_i$ with the **reaction operator** $\mathcal{R}_i^k\left(\bm{u}^k\right)$.
Such an operator is required to be a Lipschitz function and be mass conservative.
...
The transmission condition refers to the flux rate at which a compartment
component is transformed and moved to either side of the membrane. In our model,
we say that this equals to the outer flux $\mathcal{D}_i^k$ of the
component $i$ leaving the compartment $k$. It is defined by a general
**transmission condition**, $\mathcal{T}^{kl}_i$, that may take the form of
typical Neumann boundary condition as well as complex chemical reaction networks
for the different components touching the membrane, i.e.,
$$
