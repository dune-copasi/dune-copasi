...
Here, we follow the [`DUNE` ini file convention](ini_file) to refer to
...
import Trace from '!!raw-loader!../../../src/options/trace.path';

<CodeBlock language="bash">trace.path: {Trace}</CodeBlock>
...
See [Input Data](input_data) section for more information about usage.
...
Expressions in this section may contain [Input Data](input_data) functions.
