...
make_parser(const std::string& expression,
            ParserType parser_type = default_parser)
{
  if (parser_type == ParserType::MuParser) {
#if HAVE_MUPARSER
    return std::make_unique<MuParser>(expression);
#else
...
#if HAVE_SYMENGINE
    return std::make_unique<SymEngineParser>(expression, SymEngineParser::Type::Native);
#else
...
#if HAVE_SYMENGINE
    return std::make_unique<SymEngineParser>(expression, SymEngineParser::Type::SBML);
#else
    DUNE_THROW(Exception, "ExprTkParser is not available");
#endif // HAVE_SYMENGINE
  }
...
#if HAVE_EXPRTK
    return std::make_unique<ExprTkParser>(expression);
#else
    DUNE_THROW(Exception, "ExprTkParser is not available");
#endif // HAVE_EXPRTK
  }
