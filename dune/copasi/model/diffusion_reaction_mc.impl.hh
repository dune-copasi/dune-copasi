  ...
  const ParameterTree& config) const -> std::unique_ptr<PDELab::OneStep<State>>
{
  ...
  const auto& basis = any_cast<const MultiCompartmentBasis&>(state.basis);
  std::shared_ptr one_step = make_diffusion_reaction_step_operator<LocalBasisTraits, Coefficients, Residual, ResidualQuantity, TimeQuantity>(config, basis, 2, _functor_factory, _coarse_cell_data);
