...
struct PDELabFunction
{
  ...
  auto require(F&& f) -> decltype(
    ...
    f.evaluate(
      std::declval<const typename F::Traits::DomainType&>(), 
...
struct PDELabGridFunctionTraits : Refines<PDELabFunctionTraits>
{
  ...
  auto require(GF&& gf) -> decltype(
    requireType<typename GF::Traits::GridViewType>(),
...
struct PDELabGridFunction
{
  ...
  auto require(GF&& gf) -> decltype(
    ...
    gf.evaluate(
      std::declval<const typename GF::Traits::DomainType&>(), 
...
struct PDELabGlobalCallable
{
  ...
  template<class F>
  auto require(F&& f)
    -> decltype(f( std::declval<Domain>())
  );
};

/**
 * @brief     Check if a type is a global callable usable as a function in PDELab
...
/**
 * @brief     Check if a type is a local or global callable usable as a function in PDELab
