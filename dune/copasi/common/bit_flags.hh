...
namespace Dune::Copasi {
  ...
  //! Bitflag indicator
  // This class might be specialized when a certain enum is wanted to do not have automatic convertion to bitflags
  template<class T>
   ...
   * @details   This class allows that an enum can be operated as a bit set of flags.
   *            When an enum is a bit flag most operators expected for bits are enabled
   *            to operate directly on that enum. This object is different than a std::bitset
   *            because it is fully constexpr and because operators on the underlying enum are
   *            automatic.
   * @code
   *   // Notice that each option correspond to a different bit
   *   enum class MyEnum {option_a = 0b01, option_b = 0b10};
   *   // `my_enum` is deduced as `BitFlags<MyEnum>` type because operator | on `MyEnum` was overloaded to do so
  ...
  class BitFlags
  {
    ...
    static_assert(is_bit_flag_v<Enum>, "Enum is not a bit flag and it should not be instatiated in BitFlag class");
    ...
    static constexpr UnderlyingType null_value = UnderlyingType{} ^ UnderlyingType{};

  public:

    //! Bitflag with all flags turned on
    static constexpr BitFlags<Enum> All = BitFlags<Enum>{static_cast<Enum>(~null_value)};

    //! Bitflag with all flags turned off
    static constexpr BitFlags<Enum> None = BitFlags<Enum>{static_cast<Enum>(null_value)};

    //! Default constructor
    // All flags are default initialized to false
    constexpr inline BitFlags() : _value{null_value} {}

    //! Enum constructor
    // Initialization with an undelying enumerator
    constexpr inline BitFlags(const Enum& value) : _value(static_cast<UnderlyingType>(value)) {}
    ...
    {
      static_assert(sizeof(UnderlyingType) <= sizeof(unsigned long long), "Underlying type is longer than maximum supported cast");
    }

    //! Return bitflag as bitset
    constexpr inline BitSet as_bitset() const {return BitSet{static_cast<unsigned long long>(_value)}; }
    ...
    constexpr inline const Enum as_enum() const {return static_cast<const Enum>(_value); }
    ...
    constexpr inline BitFlags operator| (const BitFlags& rhs) const { return static_cast<Enum>(_value | rhs._value); }
    ...
    constexpr inline BitFlags operator<< (std::size_t pos) const { return static_cast<Enum>(_value << pos); }

    //! Bitwise right shift operator
    constexpr inline BitFlags operator>> (std::size_t pos) const { return static_cast<Enum>(_value >> pos); }

    //! Bitwise or operator with this bitflag
    inline BitFlags& operator|=(const BitFlags& rhs) { _value |= rhs._value; return *this; }
    ...
    inline BitFlags& operator<<= (std::size_t pos) { _value <<= pos; return *this; }

    //! Bitwise right shift operator with this bitflag
    inline BitFlags& operator>>= (std::size_t pos) { _value >>= pos; return *this; }

    //! Bitwise not operator of this bitflag
    constexpr inline BitFlags operator~ () const { return BitFlags{static_cast<Enum>(~_value)}; }

    //! Test if the required flag is active in the bitflag
    constexpr inline bool test(const BitFlags& flag) const { return (_value & flag._value) == flag._value; }
    ...
    inline void set(const BitFlags& flag, bool value = true) { value ? (_value |= flag._value) : reset(flag); }
  ...
  operator|(Enum lhs, Enum rhs){return BitFlags<Enum>{lhs} | BitFlags<Enum>{rhs};}
