#ifndef DUNE_COPASI_OPERATOR_FORWARD_RUNGE_KUTTA_HH
#define DUNE_COPASI_OPERATOR_FORWARD_RUNGE_KUTTA_HH
...
class RungeKutta : public OneStep<std::ranges::range_value_t<DomainStages>> {
  using Inverse = Dune::Copasi::Inverse<DomainStages, RangeStages>;
  using Forward = Dune::Copasi::Operator<DomainStages, RangeStages>;
...
public:
  ...
  PDELab::ErrorCondition apply(const Domain& start_stage, Domain& final_stage) override
  {
    Domain start_stage_copy = start_stage;
    return this->apply(start_stage_copy, final_stage);
  }

  PDELab::ErrorCondition apply(Domain& start_stage, Domain& final_stage) override
  {
...
private:
  PDELab::ErrorCondition applyFullyImplicit(Domain& start_stage, Domain& final_stage) {
  ...
  PDELab::ErrorCondition applySemiImplicit(Domain& start_stage, Domain& final_stage) {
    TRACE_EVENT("dune", "RungeKutta");
    auto& tableau = getInstationaryCoefficients();
    auto& forward = getForward();
    auto& inverse = getInverse();
...
#endif // DUNE_Copasi_OPERATOR_FORWARD_RUNGE_KUTTA_HH
