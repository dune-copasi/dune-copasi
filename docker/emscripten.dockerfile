...
RUN mkdir oneTBB/build \
    ...
    && cmake oneTBB -B oneTBB/build -G Ninja $DEFAULT_CMAKE_FLAGS -DTBB_STRICT=OFF -DCMAKE_CXX_FLAGS=-Wno-unused-command-line-argument -DTBB_DISABLE_HWLOC_AUTOMATIC_SEARCH=ON -DTBB_EXAMPLES=OFF -DTBB_TEST=OFF
