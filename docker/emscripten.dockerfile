...
ENV CMAKE_CXX_FLAGS="'-fexceptions'"
...
RUN mkdir oneTBB/build \
    ...
    && cmake oneTBB -B oneTBB/build -G Ninja -DBUILD_SHARED_LIBS=OFF -DTBB_STRICT=OFF -DTBB_DISABLE_HWLOC_AUTOMATIC_SEARCH=ON -DTBB_EXAMPLES=OFF -DTBB_TEST=OFF
...
RUN mkdir libtiff/build-tiff \
    && source /duneci/modules/emsdk/emsdk_env.sh \
    && cmake libtiff -B libtiff/build-tiff -DBUILD_SHARED_LIBS=OFF -Dtiff-tests=OFF
...
RUN mkdir fmt/build \
    && source /duneci/modules/emsdk/emsdk_env.sh \
    && cmake fmt -B fmt/build -DBUILD_SHARED_LIBS=OFF -DFMT_TEST=OFF
...
RUN mkdir spdlog/build \
    && source /duneci/modules/emsdk/emsdk_env.sh \
    && cmake spdlog -B spdlog/build -DBUILD_SHARED_LIBS=OFF -DSPDLOG_BUILD_TESTS=OFF -DSPDLOG_FMT_EXTERNAL=ON
...
RUN echo CMAKE_FLAGS+=\"\
 ...
 -DCMAKE_EXE_LINKER_FLAGS=\'-sEXPORTED_RUNTIME_METHODS=ccall,cwrap,FS,callMain -sINVOKE_RUN=0 -sALLOW_MEMORY_GROWTH=1 -sEXPORT_ES6=1 -sMODULARIZE -sEXPORT_NAME=wasm -sENVIRONMENT=web -sPROXY_TO_PTHREAD=1 -fexceptions \' \
